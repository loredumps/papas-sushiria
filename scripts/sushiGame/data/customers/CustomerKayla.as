package sushiGame.data.customers
{
   import sushiGame.data.CustomerDataFile;
   import sushiGame.data.CustomerOrder;
   import sushiGame.data.FoodData;
   import sushiGame.data.GameData;
   
   public class CustomerKayla extends CustomerDataFile
   {
       
      
      public function CustomerKayla()
      {
         super();
      }
      
      override public function initCustomerData() : void
      {
         this.customerName = "Kayla";
         this.customerClipName = "Kayla";
         this.customerFirstGame = "Papa\'s Burgeria";
         this.thumbColor = "pink";
         this.flipdeckNumber = 143;
         this.hasAlternateHands = true;
         this.setupOrder();
         this.stickers = ["mangoby","nuttybutter","blueribbon"];
      }
      
      override public function setupOrder() : void
      {
         var _loc1_:CustomerOrder = new CustomerOrder();
         _loc1_.setPaper(FoodData.PAPER_MOMOIRO);
         _loc1_.setRice(FoodData.RICE_BLACK,true);
         _loc1_.addFillings(FoodData.FILLING_UNAGI,FoodData.FILLING_LOBSTER);
         _loc1_.addToppings(FoodData.DROP_PRAWN,FoodData.SHAKER_TEMPURACRUNCH,FoodData.SAUCE_DUCK);
         _loc1_.setTea(FoodData.TEA_STRAWBERRY,FoodData.BUBBLE_WATERMELON);
         this.addCustomerOrder(_loc1_);
         var _loc2_:CustomerOrder = new CustomerOrder(GameData.MONTH_APRIL);
         _loc2_.setPaper(FoodData.PAPER_SHIROI);
         _loc2_.setRice(FoodData.RICE_BLACK,true);
         _loc2_.addFillings(FoodData.FILLING_UNAGI,FoodData.FILLING_LOBSTER);
         _loc2_.addToppings(FoodData.DROP_PRAWN,FoodData.SHAKER_TEMPURACRUNCH,FoodData.SAUCE_SWEETSAKURA);
         _loc2_.setTea(FoodData.TEA_STRAWBERRY,FoodData.BUBBLE_WATERMELON);
         this.addCustomerOrder(_loc2_);
      }
   }
}
